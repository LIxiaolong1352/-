...
（1）规范起跳落地技术。应加强起跳落地技术的训练，包括：控制助跑速度，避免过快助跑；落地时膝关节微屈（约130°-150°），利用肌肉缓冲；双脚同时落地，分散冲击力；落地后顺势屈膝缓冲，避免膝关节伸直锁定。

（2）优化急停变向技术。应加强急停变向技术的训练，包括：急停时降低重心，增大支撑面；变向时避免膝关节过度内扣；加强髋关节灵活性训练，减少膝关节代偿。

（3）改进半蹲防守姿势。应控制半蹲角度，避免长时间保持同一角度；加强股四头肌力量训练，提高半蹲位稳定性。'''
...
基于上述研究成果，建议：
（1）引入NMT热身，包括动态拉伸、平衡训练、激活训练和轻量跳跃练习。
（2）加强下肢力量训练，包括股四头肌、腘绳肌、臀肌和小腿三头肌力量训练。
（3）改善髋关节灵活性，加强髋关节灵活性和力量训练。
（4）强化核心稳定性，加强腹肌、背肌力量训练和抗旋转训练。'''
...
（1）规范热身流程。建立规范的热身流程，总热身时间应达到15-20分钟。
（2）合理安排训练负荷。遵循循序渐进原则，监控训练强度，安排充分恢复。
...
print("第四章完成，使用[15]")
print("全部15篇文献已按顺序使用完毕！")

# 第五章 结论（不再使用新文献，只用已有内容）
doc.add_heading('第五章 结论', level=1)

content_5 = f'''本研究以江汉大学大学生篮球运动参与者为研究对象，通过问卷调查和数据分析，系统研究了篮球运动膝关节损伤现状及预防策略，得出以下结论：

（1）江汉大学大学生篮球运动膝关节损伤发生率为{injured_rate:.1f}%，处于较高水平。

（2）损伤类型以半月板损伤（58.1%）和髌骨劳损（58.1%）为主；损伤主要发生在起跳落地（51.6%）和急停变向（22.6%）时。

（3）导致损伤的主要原因包括太累/疲劳（32.3%）、准备活动不充分（25.8%）和场地条件差（16.1%）。

（4）基于损伤现状和影响因素分析，本研究从技术动作优化、身体素质提升、训练管理优化、环境保障完善四个维度构建了系统的预防策略体系。
...
# 保存最终版本
output_path = r'C:\Users\Administrator\Desktop\毕业论文_刘小龙_15文献版.docx'
doc.save(output_path)
print(f"论文全部完成！已保存到: {output_path}")
print("全文只用[1]-[15]，每篇只用一次，按顺序引用！")
